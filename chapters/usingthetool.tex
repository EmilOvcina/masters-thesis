...
With the foundational knowledge and context for the microservice API patterns, the Jolie programming language, and the current tools both for Jolie and for visualization in general in place, it is time to get familiar with the tool developed for this thesis.
...
The service is created in the main file and the execution is set to \emph{concurrent}.
...
a service named \emph{Product} is created in the main \texttt{.ol} file and once again the execution context is set to \emph{concurrent}.
...
This service is created in the main file, and the execution is also \emph{concurrent}. The analytics service is added to the architecture file and is displayed in the visualization UI.
