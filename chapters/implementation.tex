...
type in the necessary information to create a new local input port for the embedded service. This is a shortcut to first creating a local port using the sidebar and then embedding using case 2.
...
The importing of service symbols is also handled here, where the embedded service will be automatically imported.
...
The service is created with one edit because no code ranges of the service exist yet, so the components cannot be created separately.

\section{Prototyping}
% generating docker-compose files
When the user executes the \textit{Docker-Compose} command from the VS Code extension or the command line, the
the \javatoolname[] is invoked via the adapter and fetches the JSON needed for creating the prototype.
First, the docker-compose file is created by simply writing the generated content into the YAML file.
\cref{appen:docker_yaml} shows an example of how the docker-compose YAML can be generated.
...
VS Code runs on Electron which uses NodeJS as runtime, meaning that a NodeJS library is used to access the filesystem to make folders, copy files and write to the YAML file.
...
% todo more about how the service names in docker-compose are chosen, and build folder names etc.